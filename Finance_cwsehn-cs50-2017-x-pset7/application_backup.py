...
def double_check():
    if request.method == "POST":
        ...
        return render_template("double_check.html", name=check_list[0][0], price=check_list[0][1], symbol=check_list[0][2], shares=check_list[0][3])
...
def buy():
    ...
    if request.method == "POST":
        ...
        if (shares * quote["price"]) <= rows[0]["cash"]:
            ...
            checkers = [name, price, symbol, shares]
    ...
    if request.method == "POST":
        ...
        while True:
            ...
            if batch <= shares:
                ...
                #new_row = db.execute("UPDATE transactions SET shares=0 WHERE user_id=:user_id AND symbol=:symbol AND shares=:shares ORDER BY shares DESC LIMIT 1", 
                          #user_id=session["user_id"], symbol=symbol.upper(), shares=batch )          
